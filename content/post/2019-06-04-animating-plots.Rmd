...
There are many instances where it may be useful to animate graphical representations of data, perhaps to better communicate how some parameter has been shown to vary with time. However, the below is a purely gimmicky example that demonstrates some of the functionality of the [*gganimate*](https://cran.r-project.org/web/packages/gganimate) library.
...
The example dataset that I've selected is the fifa 19 dataset, found on [Kaggle](https://www.kaggle.com/karangadiya/fifa19).
...
The above filter results in the `topPlayers` dataframe including the highest rated `r, size` players on the game.
...
Below shows how we can use ggplot to create a scatterplot to compare the value and weekly wage of the players.
...
This plot is the basis for the animation. Adding a function from *gganimate* and saving as a new variable:
```{r, warning = FALSE, message = FALSE}
...
`transition_states()` requires specification of the discrete states that the plot animate sequentially. 
...
I tried a few alternatives and because each state has quite a few points I wanted it to be held for a reasonable amount of time. However, it does result in a total of 1200 frames in the animation. On my laptop this required approximately 3 minutes to render. A shorter animation could be produced in a few seconds.

![](fifa.gif)

Once you are happy with your animation you can save it (in various formats).
