...
image:
  ...
  focal_point: ''
  preview_only: no
lastmod: '2020-02-15T10:30:10Z'
projects: []
slug: bayesian-regression-models-with-stan
subtitle: Modelling the Probability of Detecting Damage
tags:
- Bayesian
- MCMC
- Stan
...
* The logit function is known as a **link function** in this context.
* Logistic regression is a Bernoulli regression with a logit link function, or a Bernoulli-Logit GLM.
...
The below plot shows the size of each crack, and whether or not it was detected (in our simulation). The smallest crack that was detected was `r min((pod_df %>% dplyr::filter(det == 1))$depth)` mm deep, and the largest undetected crack was `r max((pod_df %>% dplyr::filter(det == 0))$depth)` mm deep Even so, it's already clear that larger cracks are more likely to be detected than smaller cracks, but just about all we can say at this stage.

After fitting our model, we will be able to predict the probability of detection for any crack size.
...
I've extended the above model to also included a probability of false indication in a recent piece of work.


#### Fitting the model

Data can be pre-processed in any language for which a `Stan` interface has been developed. This includes, `R`, `Python`, and `Julia`. In this example we will use `R` and the accompanying package, `rstan`. 
...
Once we have our data, and are happy with our model, we can set off the Markov chains. There are plenty of opportunities to control the way that the `Stan` algorithm will run, but I won't include that here, rather we will mostly stick with the default arguments in `rstan`.
...
PoD_extracted_params <- PoD_extracted_samples %>% 
...
for(i in seq(from = min_depth, to = max_depth, length.out = N_points)){
     ...
     newRow <- data.frame(crack_depth, logOdds, PoD); logOdds_df <- rbind(logOdds_df, newRow)
 }

# head(x = logOdds_df, n = 5)

```

```{r, echo = FALSE, message = FALSE, warning = FALSE}
ggplot(logOdds_df)+
  geom_point(aes(x = crack_depth, y = logOdds, col = ' Log Odds Scale  '), shape = 1, size = 3)+
  geom_point(mapping = aes(x = crack_depth, y = PoD, col = ' Probability Scale  '), shape = 1, size = 3)+
